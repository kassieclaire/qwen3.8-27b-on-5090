...
PORT NOTE: the OSCAR fork also had a decode-flush clip variant in
``gpu_flush_int2``, used only by the mixed-KV (HP window) pool. That module is
deliberately NOT part of this port (it is the spec-decoding-incompatible path),
and nothing here references it.
"""
...
# PORT NOTE: upstream path was ``sglang.srt.mem_cache.kv_quant_kernels``.
# Repointed to the vendored copy so this module imports standalone and does not
# depend on the OSCAR fork's memory_pool package layout.
